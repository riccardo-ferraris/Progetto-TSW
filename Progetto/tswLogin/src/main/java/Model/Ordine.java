...
public class Ordine {
	...
	double totale;
	Date data;
	List<ProdottoInCarrello> articoliOrdine;
	...
	public Ordine(String codice, String utente, String nomeF, String cognomeF, String indirizzoF, String statoF, String cityF,
			...
			Date data, List<ProdottoInCarrello> articoliOrdine) {
		super();
		this.codice = codice;
		this.utente = utente;
	...
	public List<ProdottoInCarrello> getArticoliOrdine() {
		return articoliOrdine;
	}

	public void setArticoliOrdine(List<ProdottoInCarrello> articoliOrdine) {
		this.articoliOrdine = articoliOrdine;
	}
	...
	public String creaFattura(String path) throws ClassNotFoundException, SQLException {
		...
		try {
			PDPageContentStream cs = new PDPageContentStream(doc, page); //Prepare Content Stream
		
			cs.beginText();
			cs.setFont(font, 20);
			cs.newLineAtOffset(250, 750);
			cs.showText("Perspective Art"); //Shop name
			cs.endText();
			...
			cs.close(); //Close the content stream
			doc.save(path + "/fattura" + this.getCodice() + ".pdf"); //Save the PDF
			    doc.close();
